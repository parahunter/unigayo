...
class WaveformView(wx.ScrolledWindow):
	...
	def OnMouseDown(self, event):
		...
		if (self.doc is not None) and (self.voice.sound is not None):
			...
			if (self.selectedPhrase is None) and (self.selectedWord is None) and (self.selectedPhoneme is None):
				...
				self.oldFrame = 0
				self.voice.sound.PlaySegment(float(self.scrubFrame) / float(self.doc.fps), 15.0 / self.doc.fps, 1.0)
				...
				if self.selectedPhrase is not None:
					playSegment = True
					self.voice.sound.PlaySegment(float(self.selectedPhrase.startFrame) / float(self.doc.fps),
							float(self.selectedPhrase.endFrame - self.selectedPhrase.startFrame + 1) / self.doc.fps,
							1.0)
				elif self.selectedWord is not None:
					playSegment = True
					self.voice.sound.PlaySegment(float(self.selectedWord.startFrame) / float(self.doc.fps),
							float(self.selectedWord.endFrame - self.selectedWord.startFrame + 1) / self.doc.fps,
							1.0)
				elif self.selectedPhoneme is not None:
					playSegment = True
					self.voice.sound.PlaySegment(float(self.selectedPhoneme.frame) / float(self.doc.fps),
							1.0 / self.doc.fps,
							1.0)
				self.isDragging = False
				self.draggingEnd = -1 # which end of the object (beginning or end) are you dragging
				...
				if playSegment:
					frame = -1
					while self.voice.sound.IsPlaying():
						nextFrame = int(math.floor(self.voice.sound.CurrentTime() * self.doc.fps))
						if frame != nextFrame:
							frame = nextFrame
	...
	def OnMouseMove(self, event):
		if self.isDragging:
			...
			if self.selectedPhrase is not None:
				if self.draggingEnd == 0:
					if frame != self.selectedPhrase.startFrame:
						...
						self.doc.currentVoice.RepositionPhrase(self.selectedPhrase, self.voice.soundDuration - 1)
			...
			if (frame != self.scrubFrame) and (self.doc is not None) and (self.voice.sound is not None): # and (not self.voice.sound.IsPlaying()):
	...
	def SetDocument(self, doc):
		...
		self.maxWidth = 32
		self.maxHeight = 32
		if (self.voice is not None) and (self.voice.sound is not None):
			self.frameWidth = self.sampleWidth * self.samplesPerFrame
			duration = self.voice.sound.Duration()
			time = 0.0
			sampleDur = 1.0 / self.samplesPerSec
			maxAmp = 0.0
			while time < duration:
				self.numSamples = self.numSamples + 1
				amp = self.voice.sound.GetRMSAmplitude(time, sampleDur)
				self.amp.append(amp)
				if amp > maxAmp:
					maxAmp = amp
				time = time + sampleDur
			...
			self.maxWidth = self.voice.soundDuration * self.frameWidth
			self.maxHeight = self.GetClientSize().height
		self.SetVirtualSize((self.maxWidth, self.maxHeight))
	...
	def UpdateDrawing(self, redrawAll = True):
		...
		if (self.doc is not None) and (self.voice.sound is not None):
			...
			elif self.voice.sound.IsPlaying() and (not redrawAll):
	...
	def Draw(self, dc):
		...
		if (self.doc is not None) and (self.voice.sound is not None) and self.voice.sound.IsPlaying() and (not self.isDragging):
